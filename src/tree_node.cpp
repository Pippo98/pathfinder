...
Node::Node() { this->m_parent = nullptr; }
...
Node &Node::remove_child(const Node &child) {
	for (Node *current : this->m_children) {
		if (*current == child) {
			this->m_children.erase(std::remove(this->m_children.begin(), this->m_children.end(), current),
														 this->m_children.end());
			break;
		}
	}
...
bool Node::operator==(const Node &other) const { return this->m_point == other.m_point; }
bool Node::operator!=(const Node &other) const { return !(*this == other); }

bool Node::is_root() const { return this->m_parent == nullptr; }
bool Node::is_leaf() const { return this->m_children.empty(); }

double Node::distance(const Node &other) const { return this->m_point.distance(other.m_point); }
