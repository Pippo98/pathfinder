...
RRTStar2::RRTStar2() : m_bounds(), m_obstacles(), m_config() {}
RRTStar2::RRTStar2(const Rectangle &bounds, const std::vector<Polygon> &obstacles, RRTConfig config)
		: m_bounds(bounds), m_obstacles(obstacles), m_config(config) {}

void RRTStar2::set_bounds(const Rectangle &bounds) { m_bounds = bounds; }
void RRTStar2::set_obstacles(const std::vector<Polygon> &obstacles) { m_obstacles = obstacles; }
void RRTStar2::set_config(RRTConfig config) { m_config = config; }
...
std::vector<Node> RRTStar2::find_path(const Point &start, const Point &goal) {
...
Tree RRTStar2::tree() const { return m_tree; }

Point RRTStar2::m_sample() {
...
Node &RRTStar2::m_nearest_node(const Point &sample) {
...
Point RRTStar2::m_nearest_point(const Point &sample) { return m_nearest_node(sample).point_ref(); }
...
std::vector<Node *> RRTStar2::m_near(const Point &sample, double radius) {
